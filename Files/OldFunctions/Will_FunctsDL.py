    ...
    if geneoff != [] or geneon != []:
        ...
        for data, initfunc in initializer_new:
            ...
            for i in range(timecourse):
                ...
                # This if statement will stop the computation once a steady state has been identified. Markedly improves
                # compute times.
                if e[1] < 0:
                    pass
                elif e[1] >= 1:
        ...
        for data, initfunc in initializer:
            ...
            # the dynamic length (DL) iterator for this function. Will search for a steady state
            for i in range(timecourse):
                model.iterate(steps=i)
                e = model.detect_cycles()
                # This if statement will stop the computation once a steady state has been identified. Markedly improves
                ...
                if e[1] < 0:
                    ...
                    # Initialize the data entry for d[n], append cycle length and index
                    d.append(list(model.detect_cycles()))

                    # Append the model fingerprint, first and last state
                    d[n].append(model.fp())
                    d[n].append(model.first)
                    d[n].append(model.last)

                    # Collects only the last x  values and averages them for the average on state of gene, where
                    # X is the cycle length. Ensures that we only sample the outcomes of the steady states, not
                    # the path taken to get to the steady state.
                    detect_states = 0 - e[1]
                    if detect_states == 0:
                        pass
                    elif detect_states != 0:
                        coll.collect(states=model.states[detect_states:], nodes=nodes)
